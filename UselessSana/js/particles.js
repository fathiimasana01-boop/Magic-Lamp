...
class ParticleEngine {
    ...
    // Initialize floating ambient smoke clouds
    initSmoke() {
        this.smokeWisps = [];
        for (let i = 0; i < 15; i++) {
    ...
    // Emit rubbing spark particles at cursor location
    ...
    // Emit smoke puff at lamp nozzle
    emitSpoutSmoke(x, y) {
        this.smokeWisps.push({
            x: x,
            y: y,
    ...
    // Magic Explosion Burst (when Aladdin is summoned)
    triggerGoldenExplosion(cx, cy) {
        this.isSummoningVortex = true;
        this.vortexCenter = { x: cx, y: cy };

        // 150 golden & cyan sparkle particles bursting outwards
        for (let i = 0; i < 150; i++) {
            const angle = Math.random() * Math.PI * 2;
    ...
    animate() {
        ...
        // 2. Draw & Update Ambient Smoke Wisps
        ...
        // 3. Draw & Update Sparkle Particles
...
// Instantiate global particle engine
