/* ==========================================================================
   ...
   Loads dynamic responses from `data/wishes.json` with instant fallbacks.
   ========================================================================== */
...
class AladdinAIEngine {
    ...
    // Dynamically fetch JSON data file for GitHub Pages hosting
    async loadJSONData() {
        try {
            const res = await fetch('data/wishes.json');
            if (res.ok) {
                this.externalData = await res.json();
                console.log("Aladdin loaded live wishes JSON data!", this.externalData);
            }
        } catch (e) {
            console.warn("Using built-in response dataset fallback.", e);
        }
    }
    ...
    matchCategoryResponse(wish) {
        // Use dynamic external JSON if loaded
        if (this.externalData && this.externalData.categories) {
            if (wish.includes('rich') || wish.includes('money') || wish.includes('cash') || wish.includes('dollar') || wish.includes('crore') || wish.includes('rupee')) {
                const list = this.externalData.categories.money;
                const text = list[Math.floor(Math.random() * list.length)];
                return { text, expression: 'laughing' };
            }
            if (wish.includes('girlfriend') || wish.includes('love') || wish.includes('date')) {
                const list = this.externalData.categories.love;
                const text = list[Math.floor(Math.random() * list.length)];
                return { text, expression: 'neutral' };
            }
            if (wish.includes('exam') || wish.includes('pass') || wish.includes('study')) {
                const list = this.externalData.categories.exams;
                const text = list[Math.floor(Math.random() * list.length)];
                return { text, expression: 'shocked' };
            }
        }
